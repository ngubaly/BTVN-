...
void check_input_start(void){
    char from_terminal;
    uart_puts(UART_ID_1, "Type 's' to start\n\r");

    while(true){
        if(uart_is_readable(UART_ID_1)){
            from_terminal = uart_getc(UART_ID_1);
            if(from_terminal == 's'){
                uart_puts(UART_ID_1, "Input assembly command\n\r");
                break;
            }else{
                continue;
            }
...
void store_command(void){
        ...
        if(uart_is_readable(UART_ID_1)){
            if(i < 48){
                ...
                if(from_terminal == 0x0d){
                    ...
                    uart_putc(UART_ID_1, from_terminal); // callback
                    from_terminal -= '0';
                    if(j == 0){
                    ...
                    uart_putc(UART_ID_1, from_terminal);
                    ...
                    uart_puts(UART_ID_1, "Input Error\n\r");
                    break;
                }
            }else{
                break;
    ...
    uart_puts(UART_ID_1, "Store complete\n\r");
}
